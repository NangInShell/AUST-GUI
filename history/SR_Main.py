...
class video_ffmpeg_message(QObject):
    def __init__(self, encode, vcode, yuv, profile, preset, crf, bitrate, crf_or_bit, free, vformat):
        self.Encode = encode
        self.Vcode = vcode
        self.Yuv = yuv
...
class Video_SR_Thread(QThread):
    ...
    def __init__(self, v_SR_object, v_ff_object):
        ...
        self.Encode = v_ff_object.return_encode()
    ...
    def run(self):
        for video_name in self.Video_path:
            ...
            command_in = [FFMPEG_BIN,
                          ...
                          # '-color_range','tv',
                          '-sws_flags', 'accurate_rnd+full_chroma_int+bitexact',
                          '-vf','scale=out_color_matrix=bt2020nc,format=yuv444p10le',
                          '-c:v', self.Vcode,
...
class MyMainForm(QMainWindow, Ui_Form):
    ...
    def video_SR_click(self):
        ...
        encode = self.Box_encode.currentText()
        ...
        # 压制参数设置
        Video_ffmpeg_message = video_ffmpeg_message(encode, vcode, yuv, profile, preset, crf, bitrate, crf_or_bit, free,
                                                    vformat)
