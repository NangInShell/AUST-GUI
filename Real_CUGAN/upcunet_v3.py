...
if __name__ == "__main__":
    ...
    # for weight_path, scale in [("weights_v3/up2x-latest-denoise2x.pth", 2),("weights_v3/up3x-latest-denoise3x.pth", 3),("weights_v3/up4x-latest-denoise3x.pth", 4),]:
    #      for tile_mode in [0,5]:
    #          for cache_mode in [0,1,2,3]:
    #              for alpha in [0.75,1,1.15,1.3]:
    weight_path = "weights_pro/pro-conservative-up2x.pth"#超分模型路径
    scale = 2
    tile_mode = 4
    cache_mode = 3
    alpha = 1.2  # 越小越锐
    upscaler2x = RealWaifuUpScaler(scale, weight_path, half=False, device="cuda:0")
    # input_dir="%s/huge-test"%root_path
    # input_dir="%s/input_dir1"%root_path
    input_dir = "%s/tmp_frames" % root_path#源文件夹
    # output_dir="%s/opt-huge"%root_path
    output_dir = "%s/out_frames" % root_path#输出文件夹
    ...
    for name in os.listdir(input_dir):
        ...
        suffix = tmp[-1]  # 图片后缀名
        prefix = ".".join(tmp[:-1])  # 图片前缀名
        # tmp_path = os.path.join(root_path, "tmp", "A%s.%s" % (int(time.time() * 1000000), suffix))

        # print(inp_path, tmp_path)
        # 支持中文路径
        # os.link(inp_path, tmp_path)  # win用硬链接，创建文件的拷贝
        # os.symlink(inp_path, tmp_path)#linux用软链接
        # frame = cv2.imread(inp_path)[:, :, [2, 1, 0]]  # 读取文件信息
        frame = cv2.imread(inp_path)[:, :, [2, 1, 0]] # 读取文件信息
        t0 = ttime()
        ...
        # pdb.set_trace()
        # torch.cuda.empty_cache()
        # tmp_opt_path = os.path.join(root_path, "tmp", "B%s.%s" % (int(time.time() * 1000000), suffix))
        final_opt_path = os.path.join(output_dir, prefix + '.' + 'png')#在这里更改jpg或png格式
